...
If you are using MobaXTerm, an alternative way of logging in to the server is shown in the [MobaXTerm demo](https://mobaxterm.mobatek.net/demo.html).
...
*Note:* If you find that your connection to the server keeps dropping out, you can use a tool such as [tmux](https://www.ocf.berkeley.edu/~ckuehl/tmux/) or [screen](https://www.gnu.org/software/screen/) to create a persistent session.
...
+ <kbd>Ctrl/Meta+f</kbd> to move forward one character (<kbd>Ctrl+f</kbd>) or one word (<kbd>Meta+f</kbd>)
    + The *Meta* key is generally the <kbd>Alt</kbd> key, or the *option* key on a Mac
...
+ <kbd>Ctrl/Meta+b</kbd> to move back one character (<kbd>Ctrl+b</kbd>) or one word (<kbd>Meta+b</kbd>)
...
+ Scrolling through the history with the up and down arrows (or <kbd>Ctrl+p</kbd> and <kbd>Ctrl+n</kbd>)
...
<br>
One way would be to press <kbd>Ctrl+r</kbd>, then type `date`, then press <kbd>Ctrl+r</kbd> again repeatedly until you see the command, then press <kbd>Enter</kbd> to run it. 
<br>
...
<br>
A third way would be to either hit the up arrow or type <kbd>Ctrl+p</kbd> to scroll back through the history until you see the command, then press <kbd>Enter</kbd> to run it.
</div>
</details>
</div>
